...
def emissionline_model(pars, x):
    ...
    lya_amp = pars[19] # M.D.R 01/06/2021
    dz_lya = pars[20] # M.D.R 01/06/2021
    ...
    lya_1216_obs = 1215.67 * (1+z+dz_lya) # M.D.R 01/06/2021
    #lya_obs = 1215.67 * (1+z) # M.D.R 01/06/2021
    lya_obs = 1215.67 * (1+z+dz_lya) # M.D.R 01/06/2021

    oii_3727_obs = 3727.09 * (1+z+dz_oii) #3727.0 * (1+z+dz_oii) - M.D.R 01/06/2021
    oii_3730_obs = 3729.88 * (1+z+dz_oii) #3730.0 * (1+z+dz_oii) - M.D.R 01/06/2021
    hg_obs = 4341.68 * (1+z + dz_hg) #4342.0 * (1+z + dz_hg) - M.D.R 01/06/2021
    hb_obs = 4862.68 * (1+z + dz_oiii) #4863.0 * (1+z + dz_oiii) - M.D.R 01/06/2021
    oiii_4959_obs = 4960.30 * (1+z+dz_oiii)# 4960.0 * (1+z+dz_oiii) - M.D.R 01/06/2021
    oiii_5007_obs = 5008.24 * (1+z+dz_oiii) #5008.0 * (1+z+dz_oiii) - M.D.R 01/06/2021
    nii_6550_obs = 6549.85 * (1+z) #6550.0 * (1+z) - M.D.R 01/06/2021
    ha_obs = 6564.61 * (1+z) #6565.0 * (1+z) - M.D.R 01/06/2021
    nii_6583_obs = 6585.28 * (1+z) #6585.0 * (1+z) - M.D.R 01/06/2021
    sii_6716_obs = 6718.29 * (1+z) #6718.0 * (1+z) - M.D.R 01/06/2021
    sii_6731_obs = 6732.67 * (1+z) #6733.0 * (1+z) - M.D.R 01/06/2021
    ...
    #### start adding lines:
    ### M.D.R 01/06/2021 ###
    if lya_obs > transition_wave:
       sigma = sigma_red
    else:
        sigma = sigma_blue

    w=np.where( (x > lya_obs - fit_region) & (x < lya_obs + fit_region))
    if np.size(w) > 0:
        line_lya = lya_amp  * gaussian(x[w], lya_obs, sigma)
...
def fit_obj(input_list):
    ...
    lya_obs = 1216 * (1+z_in) # M.D.R 01/06/2021
    ha_obs = 6565 * (1+z_in)
    sii_obs  = 6725 * (1+z_in)
  ...
  #        | ((lam_spec > siii_9532_obs + line_mask) & (lam_spec < siii_9532_obs + fit_region)))
    ...
    w=np.where((lam_spec < lya_obs - fit_region) | (lam_spec > he1_obs + fit_region)) # M.D.R 01/06/2021
#    w=np.where((lam_spec < oii_obs - fit_region) | (lam_spec > he1_obs + fit_region)) # M.D.R 01/06/2021 - original code
    ...
    pguess[20] = 0   ### dz_lya # M.D.R 01/06/2021
    ...
    parinfo[20]['fixed'] = 1   # dz_lya # M.D.R 01/06/2021
...
### M.D.R 01/06/2021 ###
        if ((lya_obs > np.min(lam_spec)) & ( lya_obs < np.max(lam_spec))):
            if lya_obs > transition_wave:
                disp = disp_red
            else:
                disp = disp_blue
            wline = np.where((lam_spec > lya_obs - 5*disp) & (lam_spec < lya_obs + 5*disp))
            if np.size(wline) > 0 :
                peakval = np.max(flux_spec[wline])
                lya_amp_guess = peakval - emissionline_model(out.params, np.array([lya_obs]))  ### flam - flam_cont
                lya_amp_guess = lya_amp_guess[0]
                if lya_amp_guess < 0:
                    lya_amp_guess = 0.
            else:
                lya_amp_guess = 1
...
#
        ...
        if z_in > 2.4:
            parinfo2[5]['fixed'] = 0   # fix dz_oii  # M.D.R 01/06/2021 - was 1, try 0?
        else:
        ...
        parinfo2[19]['value'] = lya_amp_guess # M.D.R 01/06/2021
        parinfo2[20]['fixed'] = 0   # vary dz_lya # M.D.R 01/06/2021
...
### M.D.R 01/06/2021 ###
        ...
        w=np.where((lam_spec > lya_obs - fit_region) & (lam_spec < he1_obs + fit_region)) # M.D.R 01/07/2021
...
### M.D.R 01/06/2021 ###
            if ( (lya_obs > np.min(lam_spec)) & (lya_obs < np.max(lam_spec))):  ### lya covered.
                if lya_obs > transition_wave:
                    sig = out.params[8]/2.35
                    sig_err = out.perror[8]/2.35
                ...
                if lya_flux > 0:
                    ...
                    w=np.where((lam_spec > lya_obs - fwhm_guess) & (lam_spec < lya_obs + fwhm_guess))
...
### M.D.R 01/06/2021 ###
        ...
        fit_results['dz_lya'] =  out.params[20] # M.D.R 01/06/2021
...
def fitandplot(path, par, beam_number, z_nate, fwhm_nate, config):
      ...
      lya_obs = 1216 * (1 + z_nate) # M.D.R 01/06/2021
     ...
     # print model_resid(out['fit_parameters'], lam = spec[0], flux = flux, err=error
      ...
      plt.plot([lya_obs, lya_obs], [-1e-18, 6e-18], color= 'orange') # M.D.R 01/06/2021
