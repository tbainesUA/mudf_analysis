...
def find_cwt(lam, flux, err, zeros, fwhm_est_pix, beam_name, config_pars, plotflag = False):
    ...
    ''' # Commented out or else the code crashes when files don't exist. - M.D.R. - 10/08/2020
    if np.size(peaks)  > 0:
        zero_check = []
...
def loop_field_cwt():
    ...
    #print('') # - M.D.R. - 10/08/2020
    #print('Searching for data files at: ' + str(os.getcwd())) # - M.D.R. - 10/08/2020

    g102files = glob('*G102*.dat') # g102files = glob('*G102_BEAM_*A.dat') - M.D.R. - 10/08/2020
    g102files.sort()
    g141files = glob('*G141*.dat') # g141files = glob('*G141_BEAM_*A.dat') - M.D.R. - 10/08/2020
    g141files.sort()
...
###    os.system('ls *G141_BEAM_*A.dat > linelist/g141_spec.list')
    ...
    print('') # - M.D.R. - 10/08/2020
    print('Searching for default.config at: ' + str(os.getcwd())) # - M.D.R. - 10/08/2020
...
###    g141files = g141list['col1']
    ...
    print('') # - M.D.R. - 10/08/2020
    print('Searching for catalogs at: ' + str(os.getcwd()) + '/DATA/DIRECT_GRISM/fin_F*cat') # - M.D.R. - 10/08/2020
    ...
    print('') # - M.D.R. - 10/08/2020
    print('I found the following catalogs: ' + str(catalogs)) # - M.D.R. - 10/08/2020

    # the "blue" catalog will be the first filter
    cat = asciitable.read(catalogs[0]) # cat = asciitable.read(catalogs[0]) - M.D.R. - 10/08/2020
#, guess=False, fast_reader=False
    print('') # - M.D.R. - 10/08/2020
    print('Catalog opened successfully: ' + str(catalogs[0])) # - M.D.R. - 10/08/2020
    print('') # - M.D.R. - 10/08/2020
    print(cat) # - M.D.R. - 10/08/2020
    print('') # - M.D.R. - 10/08/2020
    #print(cat.type) # - M.D.R. - 10/08/2020
    #print('') # - M.D.R. - 10/08/2020
    print(cat.info) # - M.D.R. - 10/08/2020
    print('') # - M.D.R. - 10/08/2020
    print(cat.colnames) # - M.D.R. - 10/08/2020

    if len(catalogs) > 1:
        cat2 = asciitable.read(catalogs[1])
    # a_image is the same in both catalogs
    a_images = cat['A_IMAGE'] # a_images = cat['col5'] - M.D.R. - 10/08/2020
    beam_se = cat['NUMBER'] # beam_se = cat['col2'] - M.D.R. - 10/08/2020
...
###    beam_se = blue_se['col2']    #### doesn't matter if it comes from red/blue because these cats are matched.
    ...
    print('') # - M.D.R. - 10/08/2020
    print('Searching for grism files...') # - M.D.R. - 10/08/2020
    #print('') # - M.D.R. - 10/08/2020
    #print('G102 files = ' + str(g102files)) # - M.D.R. - 10/08/2020
    #print('G141 files = ' + str(g141files)) # - M.D.R. - 10/08/2020

    for filename in g102files:

        #print('') # - M.D.R. - 10/08/2020
        #print('Parsing G102 files...') # - M.D.R. - 10/08/2020

        #filename = 'Spectra/Par302_G102_BEAM_1A.dat'
        #filename = 'Spectra/MU1_994_G102.1D.dat' - M.D.R. - 10/15/2020
        ### get spectral data
        ...
        beam = float(filename.split('_')[1]) # beam = float(filename.split('_')[-1].split('A')[0]) - M.D.R. - 10/15/2020
        parno = '0' # parno = filename.split('Par')[1].split('_')[0] - M.D.R. - 10/15/2020
...
###        a_image = a_image_blue[w][0]
    ...
    for filename in g141files:
        ...
        #print('') # - M.D.R. - 10/08/2020
        #exitprint('Parsing G141 files...') # - M.D.R. - 10/08/2020

        #filename = 'Spectra/Par302_G141_BEAM_1A.dat'
        #filename = 'Spectra/MU1_994_G141.1D.dat' - M.D.R. - 10/08/2020
        spdata = asciitable.read(filename, names = ['lambda', 'flux', 'ferror', 'contam', 'zero'])
        trimmed_spec = trim_spec(None, spdata, config_pars)
        beam = float(filename.split('_')[1]) # beam = float(filename.split('_')[-1].split('A')[0]) - M.D.R. - 10/08/2020
        parno = '0' # parno = filename.split('Par')[1].split('_')[0] - M.D.R. - 10/08/2020

        #print('') # - M.D.R. - 10/08/2020
        #print('beam = ', beam) # - M.D.R. - 10/08/2020 - HST ID Number
        #print('parno = ', parno) # - M.D.R. - 10/08/2020 - MUDF
